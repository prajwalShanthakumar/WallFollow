...
mavros_msgs::PositionTarget computeTargetVel(){
	...
	if(new_vert_data){
		if(vert_lines.confidence[0] > vert_conf_threshold){		// if close to ground, move up
			
			hold_error = ( max(vert_lines.y1[0],vert_lines.y2[0]) - desired_buffer);
			hold_velocity = -1 * computePID(hold_error, prev_hold_errors, h_pid, max_vel_h);

			altitude_error = (vert_lines.dist[0] - desired_wall_dist);
	...
	// state machine:
			...
			if(vert_lines.confidence[0] > vert_conf_threshold){
				target_vel.velocity.x = hold_velocity;
				target_vel.velocity.y = nominal_vel;
